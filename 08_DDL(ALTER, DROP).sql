...
/*
	...
	ROLLBACK (트랜잭션에 있는 작업을 취소) : 한 트랜잭션에 담겨있는 변경사항들을 삭제(취소)한 후 마지막 COMMIT 시점으로 돌아감
	SAVEPOINT (임시저장) : 현재 시점에 해당 포인트명으로 임시저장 해주겠다는 의미
*/

/*
	- DDL문 (CREATE, ALTER, DROP)을 수행하는 순간 기존 트랜잭션에 있던 변경사항들은 무조건 COMMIT이 된다. (실제 DB반영이 된다.)
	- 즉, DDL문 수행 전 변경사항들이 있다면 정확하게 픽스하고 진행해야한다.
*/
